...
wandb.init(project='artificalclass', entity='chankyuu')

class Qnet(nn.Module):
    def __init__(self):
        super(Qnet, self).__init__()
        self.fc1 = nn.Linear(8, 128)  # fully connected
...
def q_learning(
        env, num_episodes=700, learning_rate=0.0001, gamma=0.99,
    ...
    for i in range(num_episodes):
        ...
        while True:
            ...
            if memory.size() > 500 and total_step_idx % train_step_interval == 0:
                ...
                training_time_steps += 1    # Q 업데이트 횟수 증가
                training_step_lst.append(training_time_steps)
                episode_reward_lst.append(last_episode_reward)

                wandb.log({"Loss ": loss, "Episode Reward": last_episode_reward, "Epsilon": epsilon})
    ...
    return training_step_lst, episode_reward_lst, \
           test_training_step_lst, test_avg_episode_reward_lst
