                ...
                if done:
                    ...
                    if self.training_time_steps > 0 and n_episode % self.test_episode_interval == 0:
                        ...
                        if all(termination_conditions):
                            print("Solved in {0} steps ({1} training steps)!".format(
                                self.total_step_idx, self.training_time_steps
                            ))
                            self.model_save(
                                test_episode_reward_avg, test_episode_reward_std
                            )
                    ...
                    if self.use_wandb:
                        self.wandb.log({
                            ...
                            "Epsilon": epsilon,
                            "Num Training Steps": self.training_time_steps,
    ...
    def q_testing(self, num_episodes):
        ...
        for i in range(num_episodes):
            ...
            while True:
                ...
                # epsiode_reward는 0:21이면 21점이고 5:21이면 16, 20:21 이면 1, 21:20 이면 -1을 받는다.
...
# 이부분 전부 false, 또는 none 인데, 학습이 잘 되도록 이걸 찾아내는 것이 숙제
def main():
    dqn = DQN(
        use_wandb=True,                         # WANDB 연결 및 로깅 유무
        wandb_entity="chankyuu",                # WANDB 개인 계정
        ...
        replay_buffer_size=100000,                # 리플레이 버퍼 사이즈
        min_buffer_size_for_training=5000,      # 훈련을 위한 최소 리플레이 버퍼 사이즈
        epsilon_start=0.7,                     # Epsilon 초기 값
        epsilon_end=0.3,                       # Epsilon 최종 값
        epsilon_scheduled_last_episode=400,    # Epsilon 최종 값으로 스케줄되어지는 마지막 에피소드
        print_episode_interval=1,               # Episode 통계 출력에 관한 에피소드 간격
        test_episode_interval=50,             # 테스트를 위한 training_step 간격
    ...
    ) #epsiode_reward_avg_solved 가 0이면 최소 6시간 걸리고 높아질수록 학습 시간이 더 올라감
    # 교수님이 한 wandb 그래프 보고 참고하면 좋을듯
    dqn.train_loop()
