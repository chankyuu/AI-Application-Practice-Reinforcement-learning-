...
def q_table_learning(num_episodes=200, alpha=0.1, gamma=0.95, epsilon=0.1, test_num_episodes=3):
    # Q-Table 초기화
    ...
    for i in range(num_episodes):
        ...
        sList = [observation]  # 에이전트가 방문한 인덱스 list
        ...
        while True:
            ...
            # training_steps에 따른 성능 테스트
            if training_time_steps % 10 == 0:
                avg_episode_reward = q_table_testing(q_table, test_num_episodes)
                test_training_step_lst.append(training_time_steps)
    ...
    return q_table, training_time_steps, episode_reward_list, test_training_step_lst, test_avg_episode_reward_lst


def q_table_testing(q_table, num_episodes):
    test_env = gym.make('FrozenLakeNotSlippery-v0')
    episode_reward_list = []
    ...
    for i in range(num_episodes):
        ...
        while True:
            ...
            if done or test_episode_step >= MAX_EPISODE_STEPS:
                break
...
def main_q_table_learning():
    ...
    q_table, training_time_steps, episode_reward_list, test_training_step_lst, test_avg_episode_reward_lst = q_table_learning(NUM_EPISODES, ALPHA, GAMMA, EPSILON, TEST_NUM_EPISODES)
